...
def _extract_renhealth_dates(text: str) -> tuple[str | None, str | None]:
    ...
    if not date_to:
        ...
        if match:
            date_to = normalize_date(match.group(1))
            ...
            if filename.lower().endswith(".pdf"):
                try:
                    ...
                    fio_patterns = [
                        ...
                        rf"Застрахованное\s+лицо\s*[:\-]?\s*({fio_word}(?:\s+{fio_word}){{1,2}})",
                    ]
                    patient_fio = None
                    ...
                    policy_number = None
                    label_patterns = [
                        r"Страховой\s+полис\b\s*[:№]?\s*",
                        r"Полис\b(?:\s*ДМС)?\s*[:№]?\s*",
                    ]
                    for label_pat in label_patterns:
                        label_match = re.search(
                            label_pat, text_spaced, flags=re.IGNORECASE
                        )
                        if not label_match:
                            continue
                        tail = text_spaced[label_match.end() : label_match.end() + 120]
                        policy_match = re.search(
                            r"([0-9A-ZА-ЯЁ][0-9A-ZА-ЯЁ\-/ ]*?/\d+)",
                            tail,
                            flags=re.IGNORECASE,
                        )
                        if policy_match:
                            candidate = re.sub(r"\s+", "", policy_match.group(1)).strip()
                            if re.search(r"[A-ZА-ЯЁ]", candidate) and re.search(
                                r"\d", candidate
                            ):
                                policy_number = candidate
                                break

                    date_from, date_to = _extract_renhealth_dates(text_spaced)
