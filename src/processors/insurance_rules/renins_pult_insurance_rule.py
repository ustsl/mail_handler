...
def renins_pult_insurance_rule(
    content: str | None,
    subject: str,
    sender: str,
        ...
        for filename, file_bytes in attachments:
            ...
            if filename.lower().endswith(".pdf"):
                try:
                    ...
                    fio_pattern = (
                        r"Застрахованный:\s+([А-ЯЁа-яё\s]+?)\s*\nДата рождения:"
                    )
                    fio_match = re.search(fio_pattern, pdf_text)

                    # Страховой полис:   001ДМС39323325/342
                    policy_pattern = r"Страховой полис:\s*([0-9A-ZА-ЯЁ/]+)"
                    policy_match = re.search(policy_pattern, pdf_text)

                    if fio_match and policy_match:
                        patient_fio = fio_match.group(1).replace("\n", " ").strip()
                        # схлопываем лишние пробелы, если ФИО поехало при парсинге
                        patient_fio = re.sub(r"\s{2,}", " ", patient_fio)

                        policy_number = policy_match.group(1).strip()

                        print(
                            f"Из PDF '{filename}' извлечено: "
                            f"ФИО='{patient_fio}', Полис='{policy_number}'"
                        )
